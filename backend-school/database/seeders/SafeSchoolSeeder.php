...
class SafeSchoolSeeder extends Seeder
{
    /**
     * Run the database seeds.
     * This version safely handles existing data without truncating.
     */
    public function run(): void
    {
        $this->command->info('Running Safe School Seeder...');
        
        // Check if schools already exist
        $existingCount = School::count();
        
        if ($existingCount > 0) {
            $this->command->info("Found {$existingCount} existing schools.");
            $this->command->info('Skipping seeding to avoid duplicates.');
            $this->command->info('Use --force flag or clear data manually if you want to reseed.');
            return;
        }
        ...
        $this->command->info('Creating ' . count($schools) . ' sample schools...');

        foreach ($schools as $schoolData) {
            try {
                // Check if school with this code already exists
                $existingSchool = School::where('code', $schoolData['code'])->first();
                
                if ($existingSchool) {
                    $this->command->warn("School with code {$schoolData['code']} already exists. Skipping...");
                    continue;
                }
                
                $school = School::create($schoolData);
                $this->command->info("✓ Created: {$school->name} ({$school->code})");
                
            } catch (\Exception $e) {
                $this->command->error("✗ Failed to create {$schoolData['name']}: " . $e->getMessage());
            }
        }

        $finalCount = School::count();
        $this->command->info("Schools seeded successfully! Total schools in database: {$finalCount}");
    }
}